...
function mergeWithInitialData(records) {
  ...
  records.forEach(item => {
    ...
    if (mapByKey.has(key)) {
      ...
      const updated = { ...existing, ...item, id: existing.id, date: normDate };
      mapById.set(existing.id, updated);
      mapByKey.set(key, updated);
    } else if (item.id && mapById.has(item.id)) {
