...
function loadData() {
    ...
    fetch(GOOGLE_SHEET_WRITE_URL)
      ...
      .then(data => {
        if (data && Array.isArray(data.records)) {
          // Preserve local posts if edited within 10 seconds
          const incoming = (Date.now() - lastLocalEditTime < 10000) 
  ...
  // Auto-sync from Google Sheet every 25 seconds (Multi-device Realtime Sync)
  setInterval(() => {
    // Pause background polling if local edit occurred within 10 seconds
    if (Date.now() - lastLocalEditTime < 10000) return;
      ...
      fetch(GOOGLE_SHEET_WRITE_URL)
        ...
        .then(data => {
          if (data && Array.isArray(data.records) && data.records.length > 0) {
            ...
            // Only re-render if posts actually changed (prevents DOM flash/glitch)
            if (!arePostsEqual(merged, calendarPosts)) {
              calendarPosts = merged;
              localStorage.setItem(STORAGE_KEY, JSON.stringify(calendarPosts));
              updateStatsRibbon();
