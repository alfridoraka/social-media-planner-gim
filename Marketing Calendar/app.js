...
function loadData() {
    ...
    fetch(GOOGLE_SHEET_WRITE_URL)
      ...
      .then(data => {
        if (data && Array.isArray(data.records)) {
          // Preserve local posts if edited within 6 seconds
          const incoming = (Date.now() - lastLocalEditTime < 6000) 
            ? [...data.records, ...calendarPosts]
            : data.records;
          calendarPosts = mergeWithInitialData(incoming);
  ...
  // Auto-sync from Google Sheet every 20 seconds (Multi-device Realtime Sync)
  setInterval(() => {
    // Pause background polling if local edit occurred within 6 seconds
    if (Date.now() - lastLocalEditTime < 6000) return;
      ...
      fetch(GOOGLE_SHEET_WRITE_URL)
        ...
        .then(data => {
          if (data && Array.isArray(data.records) && data.records.length > 0) {
            ...
            if (JSON.stringify(merged) !== JSON.stringify(calendarPosts)) {
